...
//
// The order of searching buckets is determined based on the response's preferred types (e.g., citizen, foreigner, blue card).
// - If none of the containers return a valid status, a "record not found" result is returned.
//
// Returns:
...
func ParseTumKutukResponse(respXML string) (Result, error) {
	...
	// If no buckets produced a valid status, return "record not found"
	return Result{
		Status:  false,
		Code:    2,
